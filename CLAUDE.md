...
**This is a fork.** Remotes (verify with `git remote -v` before any push): `origin` = rustdesk/rustdesk (**upstream — never push here, never open PRs from this repo**), `fork` = Ballen2270/rustdesk (**push target**; the branch on it is `patch/mhxy-spike`). Fork work happens on `patch/mhxy-spike` (headless automation: the `bridge`/`spike` binaries + MCP server, below). Upstream fixes land by merging `origin/master` in (keep fork changes additive so merges stay clean).
